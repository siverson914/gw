...
| `gw status [--json]` | One-glance cross-repo + worktree view: branch, uncommitted/untracked, ahead/behind. `--json` adds per-session unlanded work, idle time, and, for sessions opened with `--herdr`, the Herdr tab and pane and whether that pane is still open. |
...
```text
Claude:  claude-fable-5-1 claude-opus-5-5 claude-sonnet-5   claude-haiku-4-5
Codex:   gpt-6-astra      gpt-6-sol       gpt-6-luna        gpt-5.6-terra
```
...
The agent starts in a new, unfocused Herdr tab named after the session. The prompt goes over through a mode-600 file in the session dir (`.gw-launch`, removed once read), not typed into the pane, so multi-line text, quotes and `$` arrive intact. The only thing typed into the tab is `sh <gw>/gw-launch.sh <file>`, so the tab's shell doesn't need the `gw` function. gw types it only once the new shell has drawn its prompt and nothing from its rc is still running (it gives up waiting after 10s and sends it anyway, with a warning). `/gw-sessions` teaches agents this flow, including following the tab's agent with `herdr agent wait`/`read` on the returned pane id. Land or discard it later by id, as above. `herdr: true` in config never applies to `--json` or `--no-launch` starts, so orchestration scripts keep driving their own sessions unless they ask for a tab.
