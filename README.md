...
| `gw start [WT-id] [--no-continue] [--new]` | Put **every** repo on a fresh `gw/<id>` branch off `origin/<base>`, open an edit box for your prompt, pick the Claude model (←/→ · Enter; opens on the last one you used), `cd` into `.worktrees/<id>/`, and launch the agent. Pass a `WT-id` — or just run it from inside a session worktree — to **resume**, which re-enters that session and **continues the prior agent conversation** by default (`--no-continue` starts a clean conversation; `--new` forces a brand-new session even from inside a worktree). |
