...
Pass flags through, e.g. `/done --pr` (open a PR per changed repo instead of merging — your message becomes the PR title + body), `/done --no-check` (skip the gates), `/done --quick` (run each repo's lighter, diff-scoped `gateQuick` when configured — good for small, iterative lands; falls back to the full gate where none is set, so it's never less safe), or `/done --full` (force the complete gate even for repos that default to quick via `gateQuickDefault`). When it prints "merged + pushed: …" (or PR URLs), report success — listing which repos landed.
